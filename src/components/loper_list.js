...
class LoperList extends Component {
  constructor(props){
    super(props)
  ...
  exportData() {
    ...
    if (exportAll) {
      ...
      .then((response) => {
        if (response.data.ResponseCode === "200") {
          downloadCSV(response.data.ResponseData, 'loper')
        } else {
  ...
  editRow(row){
    ...
    this.props.history.push({
      pathname: `/loper_account/edit/${row.id}`
    })
  }
        ...
        <React.Fragment>
          <Row type="flex" justify="end" style={{ marginBottom : '30px' }}>
            <Col>
              ...
              {
                (this.state.create) ? <Button className='base_button primary' type="primary" onClick={()=>this.addRow()}>Add Loper</Button> : ''
              }
              
              {
                (this.state.delete) ? 
                  (
                    <Button className={
                              (Object.keys(this.state.selectedRow).length > 0) ? ("base_button delete") : ("button_disabled delete_disabled") } 
                            type="danger" 
          ...
          <BootstrapTable 
            ...
            options={{ 
              ...
              <TableHeaderColumn ref='networkCol' dataField="networkName" hidden={ !this.state.networkAgan } dataAlign="center" width='75px' dataSort={true} filter={ { type: 'TextFilter', placeholder: 'Please enter a value' }}> Network </TableHeaderColumn>
              <TableHeaderColumn ref='areaCol' dataField="areaName" dataAlign="center" width='100px' dataSort={true} filter={ { type: 'TextFilter', placeholder: 'Please enter a value' } }>Area</TableHeaderColumn>
              <TableHeaderColumn ref='statusCol' dataField="status" dataAlign="center" width='45px' dataSort={true} filter={ { type: 'TextFilter', placeholder: 'Please enter a value' } }>Status</TableHeaderColumn>
...
export default withRouter(connect(mapStateToProps, { LogUser, FormMode, SetTableStates })(LoperList));
