...
class ProductList extends Component {
  ...
  fetchData(page, size, sortName, sortOrder, filterObj) {
    ...
    let url = api_url + '/api/recent_product/?page=' + page + '&size=' + size
  ...
  exportData() {
    ...
    let url = api_url + '/api/recent_product/?page=all'
    ...
    if (exportData) {
      ...
      .then((response) => {
        if (response.data.ResponseCode === '200') {
          ...
          downloadCSV(response.data.ResponseData, 'product')
    ...
    var reload = (this.state.update) ? <a onClick={()=>this.updateStatus(row)} ><Tooltip title="Enable / Disable"><Icon type="reload" /></Tooltip></a> : ' ';
    ...
    var read = (this.state.read) ? <a onClick={()=>this.viewRow(row)} style={{ marginLeft: '5px'}}><Tooltip title="View"><Icon type="search" /></Tooltip></a> : ' ';
		...
		return <div>{reload} {edit} {read}</div>
  }
  ...
  updateStatus(row){
    let { page, size, sortName, sortOrder, filterObj, api_url, config } = this.state
    this.setState({ isLoading: true })

    let url = api_url + '/api/recent_product/' + row.id
    let data = {
      status: row.status
    }
    ...
    if (remove) {
      if (selectedData.length !== 0) {
        for(let i=0; i<selectedData.length; i++){
          let url = api_url+'/api/recent_product/?id=' + selectedData[i].id
        ...
        <React.Fragment>
          ...
          <BootstrapTable 
            ...
            options={{ 
              ...
              <TableHeaderColumn dataField='product_type' dataAlign="center" width='100px' export={true} dataSort={ true } filter={ { type: 'TextFilter', placeholder: 'Please enter a value' } }>Type</TableHeaderColumn>
              <TableHeaderColumn dataField='sku' dataAlign="center" width='150px' export={true} dataSort={ true } filter={ { type: 'TextFilter', placeholder: 'Please enter a value' } }>SKU</TableHeaderColumn>
              <TableHeaderColumn dataField='product_name' dataAlign="center" width='150px' export={true} dataSort={ true } filter={ { type: 'TextFilter', placeholder: 'Please enter a value' } }>Name</TableHeaderColumn>
              <TableHeaderColumn dataField='operator' dataAlign="center" width='150px' export={true} dataSort={ true } filter={ { type: 'TextFilter', placeholder: 'Please enter a value' } }>Operator</TableHeaderColumn>
              <TableHeaderColumn dataField='created_at' dataAlign="center" width='170px' export={true} dataSort={ true } dataFormat = {this._dateFormat.bind(this)} filter={ { type: 'DateFilter' } }>Create Date</TableHeaderColumn>
              <TableHeaderColumn dataField='start_date_time' dataAlign="center" width='170px' export={true} dataSort={ true } dataFormat = {this._dateFormat.bind(this)} filter={ { type: 'DateFilter' } }>Start Date</TableHeaderColumn>
              <TableHeaderColumn dataField='end_date_time' dataAlign="center" width='170px' export={true} dataSort={ true } dataFormat = {this._dateFormat.bind(this)} filter={ { type: 'DateFilter' } }>End Date</TableHeaderColumn>
              <TableHeaderColumn dataField='biller_name' dataAlign="center" width='150px' export={true} dataSort={ true } filter={ { type: 'TextFilter', placeholder: 'Please enter a value' } }>Host</TableHeaderColumn>
              <TableHeaderColumn dataField='harga_biller' dataAlign="center" width='100px' export={true} dataSort={ true } dataFormat = {this.curency.bind(this)} filter={ { type: 'TextFilter', placeholder: 'Please enter a value' } }>Biller Cost</TableHeaderColumn>
              <TableHeaderColumn dataField='fee_jpx' dataAlign="center" width='100px' export={true} dataSort={ true } dataFormat = {this.curency.bind(this)} filter={ { type: 'TextFilter', placeholder: 'Please enter a value' } }>Margin 0</TableHeaderColumn>
              <TableHeaderColumn dataField='fee_agent' dataAlign="center" width='100px' export={true} dataSort={ true } dataFormat = {this.curency.bind(this)} filter={ { type: 'TextFilter', placeholder: 'Please enter a value' } }>Margin 1</TableHeaderColumn>
              <TableHeaderColumn dataField='fee_loper' dataAlign="center" width='100px' export={true} dataSort={ true } dataFormat = {this.curency.bind(this)} filter={ { type: 'TextFilter', placeholder: 'Please enter a value' } }>Margin 2</TableHeaderColumn>
              <TableHeaderColumn dataField='status' dataAlign="center" width='100px' export={true} dataSort={ true } filter={ { type: 'TextFilter', placeholder: 'Please enter a value' } }>Status</TableHeaderColumn>
...
export default withRouter(connect(mapStateToProps, { LogUser, FormMode })(ProductList));
